...
export default function ComparisonStudio({
  ...
  const chartSVGContent = (
    <>
      ...
      {suppPoints && (
        <g>
          <path d={buildPath(suppPoints)} fill="none" stroke={SUPP_COLOR} strokeWidth={1.1 * s} strokeLinejoin="round" opacity={primaryIsSupplement ? 0.8 : 0.45} />
      ...
      {symptomPointSets.map((pts, idx) => (
        <g key={`sym-${idx}`}>
          <path d={buildPath(pts)} fill="none" stroke={SYMPTOM_STYLES[idx].color} strokeWidth={1.3 * s} strokeLinejoin="round" opacity={selectedSymptoms[idx] === primarySeriesId ? 0.8 : 0.45} />
